...
4. **Apply approved fixes and verify** — implement only what was approved, then run the vault's check script (or, where it has none, the discoverability bar `init-brain` step 4 sets) over the changed parts. Close the loop in the scorecard file: mark each gap as fixed or declined, so the next audit starts from what was already decided rather than re-proposing it.
