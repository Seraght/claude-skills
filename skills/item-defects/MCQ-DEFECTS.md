...
Defects that only selected-response items can carry. The defects that apply to items of every kind — congruence with the outcome, correctness of the keyed answer, and the ones that appear only across a whole paper — are in [COMMON-DEFECTS.md](./COMMON-DEFECTS.md), and every item is judged against that file as well as this one.

**The host project governs.** Where a project or an institution fixes an item rule — a required number of options, a required count of negatively worded items — an item is not flawed for following it.

Each defect names its fix. A finding without a quoted span is not a finding.
