...
Defects that any item can carry, whatever its format — a multiple-choice item, an essay prompt, a matching set, or a practical task. Every item is judged against this file, and then against the file for its own format.

**The host project governs.** Where a project or an institution fixes a rule, an item is not flawed for following it.

Group C is judged one item at a time against the course's anchors. Group D only appears once the paper is assembled, and no per-item pass can find it.
...
- **Self-containment.** The item appeals to context that exists only outside the paper — "จากที่เรียน", "ตามที่อาจารย์สอน", "in the case we discussed last week". An examinee who knows the subject but was absent, or who studied from the book, cannot answer it; what the item measures is attendance. This is not the same as a *context-dependent item set*, which is a legitimate format that supplies its context on the page. *Severity:* **Block** where the answer cannot be determined without the outside context, since the answer is not on the paper at all; **Revise** where the phrase is decorative and the item survives its deletion. *Fix:* delete the phrase, or move onto the page whatever the examinee needs — the case, the figure, the definition.
