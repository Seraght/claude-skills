...
7. **Verify before finishing** — run the project's check script where one exists, then confirm: every section reconciled against its source, the decision row appended, the frontmatter dates refreshed, and every link resolving. Cross-tree links break more often than any others in the repo, and this document holds more of them than any other.
